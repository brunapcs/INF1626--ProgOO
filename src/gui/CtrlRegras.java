...
class CtrlRegras implements Observable {
	...
	public String salvarJogo() {
		...
		jogo += "Dados: " + Arrays.toString(d.getDnum()) + "\n";
		jogo += "Position_on" + Integer.toString(position_on) + "\n";
		jogo += "Terreno_on" + terreno_on.getNome() + "\n";
		jogo += "Botoes " + bot + "\n";
		
		for(int i = 0; i < numPlayers; i++) {
			Jogador j = tab.getJogador(i);
			
			//jogo += "imagemPino " + imgToBase64String(j.getJogadorImage(), jogo) + "\n";
			...
			jogo += "coordenadas X: " + Integer.toString(j.getPosX()) +" Y: " + Integer.toString(j.getPosY()) + "\n";
			jogo += "offset " + Integer.toString(j.getOffset()) + "\n";
			jogo += "numero de Turnos Prisao " + Integer.toString(j.getNumTurnPrisao()) + "\n";
			
			for(int k=0;k<8;k++){
				jogo += "NCMC " + Integer.toString(j.getQtdCasasCor(k)) + "\n";
			}
			ArrayList<Cartas> prop = j.getPropriedades();
			
			
			for(int k = 0; k < prop.size(); k++) {
				jogo += "Nome" + prop.get(k).getNome() + "\n";
				jogo += "Proprietario " + prop.get(k).getProprietario() + "\n";
			}
		}	
		return jogo;
	}
