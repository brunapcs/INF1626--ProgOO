...
public class CartaTerreno extends Cartas{
	...
	public CartaTerreno(String n ,int cas, int hot, int a, int b , int c , int d, int e, int f , int p, int custo, String Cor){ 
		...
		cor = Cor; 
		aluguel.add(a); aluguel.add(b); aluguel.add(c); aluguel.add(d); aluguel.add(e); aluguel.add(f);
		pos = p; 
	...
	public static CartaTerreno getTerrenoPos(int p){ 
			return (CartaTerreno)cartas.get(p); 
	 }
	...
	public int getAluguel(int numCasas) {
		System.out.print("casas:" + Integer.toString(numCasas)+ " \n"); 
